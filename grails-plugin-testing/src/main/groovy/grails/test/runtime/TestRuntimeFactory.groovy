...
class TestRuntimeFactory {
    ...
    private static List<UseTestPlugin> collectUseTestPluginAnnotations(Class annotatedClazz) {
        List<UseTestPlugin> annotations = []
        Class<?> currentClass = annotatedClazz
        while(currentClass != Object) {
            UseTestPlugin useTestPlugin = currentClass.getAnnotation(UseTestPlugin)
            useTestPlugin && annotations << useTestPlugin
            useTestPlugin = currentClass.getPackage()?.getAnnotation(UseTestPlugin)
            useTestPlugin && annotations << useTestPlugin
            currentClass = currentClass.getSuperclass()
        }
    ...
    private TestRuntime findOrCreateRuntimeForTestClass(Class testClass, TestRuntimeSettings runtimeSettings) {
        ...
        if(runtime==null) {
            ...
            runtime = createRuntimeForSettings(runtimeSettings, null, false)
    ...
    private TestRuntime createRuntimeForSettings(TestRuntimeSettings runtimeSettings, TestEventInterceptor interceptor, boolean shared) {
        TestRuntime runtime
        if(runtimeSettings.requiredFeatures) {
            runtime = new TestRuntime(runtimeSettings.requiredFeatures, resolveFeaturesToPlugins(runtimeSettings), interceptor, shared)
        } else {
